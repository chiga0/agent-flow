...
| P2b | 用户管理 UI 强化 | `foundation_done` | owner 可创建、禁用、改角色、重置密码 |
...
P2b 的验收重点是让 owner 可以在页面内完成普通用户生命周期管理：

| 验收项 | 标准 |
| --- | --- |
| 创建用户 | Access -> Users 默认创建 `member`，也可选择 operator/auditor/owner |
| 角色调整 | owner 可在用户行内调整角色，后端统一校验合法角色 |
| 禁用/启用 | owner 可禁用非当前用户，禁用后旧 session 失效且无法登录 |
| 重置密码 | owner 可重置用户密码，旧密码不可再登录 |
| 安全护栏 | 不允许禁用当前 owner，避免误锁定 |
| 测试 | runtime integration test + Web unit + Web E2E 覆盖 |
